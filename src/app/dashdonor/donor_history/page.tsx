...
function AddRequestForm({ user, onClose, onSave }: { user: User; onClose: () => void; onSave: (payload: any) => void; }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setIsSubmitting(false); // Stop loading on error only
        }
    };
...
//========================================================//
// 6. MAIN PAGE COMPONENT (History Page - Updated)
//========================================================//
...
export default function DonationHistoryPage() {
    ...
    const [latestAppointment, setLatestAppointment] = useState<any | null>(null); // State for latest appointment
    const [daysLeft, setDaysLeft] = useState(84); // State for eligibility
    const router = useRouter();

    const fetchData = async () => { // Renamed to fetchData for clarity
        setLoading(true);
        ...
        if (donationError) {
            ...
            if (donationData.length > 0) {
                const lastDonationDate = new Date(donationData[0].date_received);
    ...
    const filteredDonations = donations.filter(donation =>
        ...
        donation.date.toLowerCase().includes(searchTerm.toLowerCase()) || // Search by date
        donation.donationId.toLowerCase().includes(searchTerm.toLowerCase()) // Search by ID
    );
