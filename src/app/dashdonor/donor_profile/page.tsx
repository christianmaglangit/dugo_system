...
function AddRequestForm({ user, onClose, onSave }: { user: User; onClose: () => void; onSave: (payload: any) => void; }) {
    ...
    // 1. New state para sa loading (prevent double submit)
    ...
    const [form, setForm] = useState({
        ...
        blood_component: "",
        units: 1,
        request_reason: "", // 2. Added Reason field sa state
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // 3. Prevent multiple clicks kung nag submit na
        if (isSubmitting) return;

        let requiredFieldsMissing = false;
        // 4. Apil na ang request_reason sa validation
                        ...
                        <InputField label="Confirm New Password" name="confirmPassword">
                             <div className="relative">
                                <input
                                    ...
                                    className="bg-gray-50 border border-gray-300 text-black pl-3 pr-10 h-11 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-red-500"
                                    disabled={!newPassword} // Disable if new password is empty
                                />
                                 <button
                                    type="button"
        ...
        <div className="space-y-4 text-sm">
            <div>
                ...
                <a className="flex items-center gap-2 text-gray-600 hover:underline"> {/* Replace placeholder */}
                    <PhoneIcon />
                    <span>(063) 223 1065</span> {/* Replace with actual Iligan hotline */}
                </a>
...
export default function ProfilePage() {
    ...
    const fetchUserData = async () => {
        ...
        const profileToSet = profileData || user;
        if (profileToSet) {
             setUser({
                ...
                livesSaved: livesSavedCount, // Corrected calculation
                user_id: profileToSet.user_id,
            });
        }
    ...
    useEffect(() => {
        ...
        const appointmentChannel = supabase.channel('public:appointments_profile') // Unique channel name
          .on('postgres_changes',
            ...
            (payload) => {
              ...
              fetchUserData(); // Refetch all data needed by the profile page
            }
          )
          .subscribe((status, err) => {
             if (status === 'SUBSCRIBED') console.log('Connected to profile appointment channel!');
             if(err) console.error('Profile appointment subscription error:', err);
          });
          
         // Add listener for blood_journey changes
         const journeyChannel = supabase.channel('public:blood_journey_profile_page') // Unique channel name
             .on('postgres_changes',
                 { event: '*', schema: 'public', table: 'blood_journey'},
                 // 1. GIGAMIT ANG SAKTONG TYPE IMBIS 'any'
                 (payload: RealtimePostgresChangesPayload<any>) => { 
                     const checkAndUpdate = async () => {
                         let relevantBloodBagId: string | null = null;
                         
                         // Note: Pwede gihapon 'any' diri kay ang 'old' partial,
                         // pero ang payload type sa taas kay sakto na.
                         if (payload.new && (payload.new as any).blood_bag_id) { 
                            relevantBloodBagId = (payload.new as any).blood_bag_id;
                         } else if (payload.old && (payload.old as any).blood_bag_id) { // Handle DELETE
                            relevantBloodBagId = (payload.old as any).blood_bag_id;
                         }
                         ...
                         if (relevantBloodBagId) {
                            ...
                            if (inventoryItem && inventoryItem.user_id === userId) {
                                console.log('Profile page: Relevant blood journey change received!', payload);
                                // 2. GI-AYO ANG TYPO: fetchData -> fetchUserData
                                fetchUserData(); 
                            }
                         }
                     };
             ...
             .subscribe((status, err) => {
                 if (status === 'SUBSCRIBED') console.log('Connected to profile journey channel!');
                 if(err) console.error('Profile journey subscription error:', err);
             });
        ...
        return () => {
            ...
            supabase.removeChannel(journeyChannel); // Remove journey channel
        };
    }, [router, user?.user_id]); // Depend on user_id
    ...
    const handleSaveProfile = async (profileUpdates: { name: string; contact: string; address: string; profile_image_url?: string }) => {
        ...
        try {
            ...
            setUser(prevUser => prevUser ? {
                ...
                profileImage: profileUpdates.profile_image_url || prevUser.profileImage
             } : null);
            ...
            Swal.fire("Success", "Your profile has been updated.", "success");
             ...
             // This error will be caught by the modal's handleSubmit
             throw error; 
        }
    };
