...
const Header = ({ onOpenRequest }: { onOpenRequest: () => void }) => {
    return (
...
function AddRequestForm({ user, onClose, onSave }: { user: User; onClose: () => void; onSave: (payload: any) => void; }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const file = form.request_form_file;
            const filePath = `public/requests/${user.user_id}/${Date.now()}_${file.name}`;
            const { error: uploadError } = await supabase.storage.from("blood_requests").upload(filePath, file);
            if (uploadError) throw uploadError;

            const { data: { publicUrl } } = supabase.storage.from("blood_requests").getPublicUrl(filePath);
         ...
         <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
           <form onSubmit={handleSubmit} className="relative p-8 md:p-10 overflow-y-auto max-h-[90vh]">
             ...
             <div className="space-y-4">
                 ...
                 <div className="flex justify-end gap-3 pt-4">
                     <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 font-semibold text-gray-700 transition">Cancel</button>
                     <button type="submit" className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 font-semibold text-white transition">Submit Request</button>
                 </div>
             </div>
           </form>
         </div>
    ...
    return (
        <div className="min-h-screen bg-gray-100 font-sans text-gray-900">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <Header onOpenRequest={() => setIsRequestModalOpen(true)} />
