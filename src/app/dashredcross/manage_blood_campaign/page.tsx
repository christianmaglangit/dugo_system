    ...
    return (
        <header className="fixed top-0 left-0 right-0 h-20 bg-white/80 backdrop-blur-md border-b border-gray-200/80 flex items-center justify-between px-6 z-40 md:left-72">
            <div className="flex items-center gap-4">
                {toggleSidebar && <button onClick={toggleSidebar} className="md:hidden p-2 dark:text-gray-700 ml-2 rounded-full hover:bg-gray-100"><MenuIcon /></button>}
                ...
                <Dialog.Panel className="bg-white rounded-2xl shadow-xl w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 overflow-hidden">
                    ...
                    <div className="relative p-8 md:p-10 overflow-y-auto max-h-[90vh]">
                        ...
                        <div className="space-y-4">
                            <InputField label="Campaign Title" name="title" value={form.title} onChange={(e: any) => setForm({ ...form, title: e.target.value })} placeholder="e.g., Annual Blood Drive"  className="bg-gray-50 border border-gray-300 px-3 py-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-red-500 dark:text-gray-700"/>
                            <InputField label="Location" name="location" value={form.location} onChange={(e: any) => setForm({ ...form, location: e.target.value })} placeholder="e.g., Iligan City Hall" className="bg-gray-50 border border-gray-300 px-3 py-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-red-500 dark:text-gray-700"/>
                            ...
                            <InputField label="Campaign Photo" type="file" accept="image/*" onChange={(e: any) => setForm({ ...form, photo: e.target.files?.[0] || null })} className="bg-gray-50 border border-gray-300 px-3 py-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-red-500 dark:text-gray-700"/>
                            <div>
                                <label className="block text-sm font-medium text-gray-600 mb-1">Description</label>
                                <textarea name="description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="bg-gray-50 dark:text-gray-700 border border-gray-300 px-3 py-2 rounded-lg w-full h-24 focus:outline-none focus:ring-2 focus:ring-red-500" />
                            </div>
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                            <div className="w-full md:max-w-md">
                                <div className="relative">
                                    ...
                                    <input type="text" placeholder="Search by Title, Location, or Date..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full dark:text-gray-700 pl-10 pr-4 py-2.5 border rounded-lg shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-red-400" />
                                </div>
                            </div>
